...
"""
Stage-0 pseudo-realtime validation for small-window IK.

This script replays an existing TRC file as a rolling window of marker data:

    full TRC -> rolling window -> MarkersReference -> IK solve -> latest state

It is intentionally file-backed for repeatability. Once this passes, the only
remaining architectural step is replacing the file-backed window with an
in-memory window built from realtime 3D markers.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Replay a TRC file as rolling windows for IK validation.")
...
def main() -> None:
    ...
    for window_idx, end_idx in enumerate(end_indices, start=1):
        ...
        markers_ref, _ = build_markers_reference(window_table, weights)
        solver, _ = build_ik_solver(model, markers_ref)
    ...
    print("\n[pass] Small-window IK replay is working at the basic level.")
